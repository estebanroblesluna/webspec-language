...
import org.webspeclanguage.plugin.webspecmodel.diagram.part.WebspecmodelDiagramEditor;
 ...
 * @generated
 */
public class WebspecmodelNavigatorActionProvider extends CommonActionProvider {
  ...
  private class OpenDiagramAction extends Action {
    ...
    public final void selectionChanged(IStructuredSelection selection) {
      ...
      if (selection.size() == 1) {
        ...
        if (selectedElement instanceof WebspecmodelNavigatorItem) {
          selectedElement = ((WebspecmodelNavigatorItem) selectedElement).getView();
        } else if (selectedElement instanceof IAdaptable) {
          selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
        }
        if (selectedElement instanceof Diagram) {
          Diagram diagram = (Diagram) selectedElement;
          if (DiagramEditPart.MODEL_ID.equals(WebspecmodelVisualIDRegistry.getModelID(diagram))) {
            myDiagram = diagram;
          }
        }
      }
    ...
    public void run() {
      ...
      try {
        page.openEditor(editorInput, WebspecmodelDiagramEditor.ID);
      } catch (PartInitException e) {
        WebspecmodelDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
      }
    }
