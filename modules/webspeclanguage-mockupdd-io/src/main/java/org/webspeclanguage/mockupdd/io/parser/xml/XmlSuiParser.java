 ...
 */
public class XmlSuiParser implements SuiParser<Element, SuiParserContext> {
  ...
  public Object parse(Element source, SuiParserContext parserContext, Object partiallyParsedObject) throws SuiParsingException {
    SuiModel model = parserContext.getSuiFactory().createSuiModel();
    for (Object child : source.getChildren()) {
      model.addPage((Page) parserContext.parse((Element) child));
    }
    return model;
  }
