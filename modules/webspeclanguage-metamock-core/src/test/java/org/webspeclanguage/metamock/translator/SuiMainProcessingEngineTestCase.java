...
public class SuiMainProcessingEngineTestCase extends SuiProcessingEngineTestCase {
  ...
  public void testRepetitionDetection() throws Exception{
		...
		assertEquals(0, SuiUtil.filterControlsByType(
		...
		Repetition r = (Repetition) SuiUtil.filterControlsByType(
				m.getPages().iterator().next().getControls(), Repetition.class)
					.iterator().next();
		assertEquals(r.getControls().size(), 2);
		Collection<Widget> repetitionButtons = SuiUtil.filterControlsByType(
		        r.getControls(), Button.class);
		Collection<Widget> repetitionLabels = SuiUtil.filterControlsByType(
