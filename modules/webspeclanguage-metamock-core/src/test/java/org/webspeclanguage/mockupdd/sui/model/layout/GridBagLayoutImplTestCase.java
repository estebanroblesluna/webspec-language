...
import org.webspeclanguage.mockupdd.sui.model.layout.impl.GridBagLayoutCellImpl;
import org.webspeclanguage.mockupdd.sui.model.layout.impl.GridBagLayoutImpl;
import org.webspeclanguage.mockupdd.utils.ValueHolder;
...
public class GridBagLayoutImplTestCase extends SuiTestCase {
  ...
  public void testAddWidgetsToGridBagLayout() throws GridBagLayoutException {
    ...
    Collection<Widget> widgets = gbl.getRowContent(0);
    assertTrue(widgets.contains(cell1.getWidget()));
    assertTrue(widgets.contains(cell2.getWidget()));
    assertFalse(widgets.contains(cell3.getWidget()));
    
    widgets = gbl.getRowContent(1);
    assertFalse(widgets.contains(cell1.getWidget()));
    assertFalse(widgets.contains(cell2.getWidget()));
    assertTrue(widgets.contains(cell3.getWidget()));
    
    widgets = gbl.getColumnContent(0);
    assertTrue(widgets.contains(cell1.getWidget()));
    assertFalse(widgets.contains(cell2.getWidget()));
    assertTrue(widgets.contains(cell3.getWidget()));
    
    widgets = gbl.getColumnContent(1);
    assertFalse(widgets.contains(cell1.getWidget()));
    assertTrue(widgets.contains(cell2.getWidget()));
    assertTrue(widgets.contains(cell3.getWidget()));
    ...
    assertSame(gbl.getCell(0, 0), cell1);
    assertSame(gbl.getCell(0, 1), cell2);
    assertNull(gbl.getCell(0, 2).getWidget());
    assertSame(gbl.getCell(1, 0), cell3);
    assertSame(gbl.getCell(1, 1), cell3);
    assertNull(gbl.getCell(1, 2).getWidget());
    ...
    gbl.removeWidget(1, 1);
    assertSame(gbl.getCell(0, 0), cell1);
    assertSame(gbl.getCell(0, 1), cell2);
    assertNull(gbl.getCell(0, 2).getWidget());
    assertNull(gbl.getCell(1, 0).getWidget());
    assertNull(gbl.getCell(1, 1).getWidget());
    assertNull(gbl.getCell(1, 2).getWidget());
    assertSame(gbl.getCell(2, 0), cell4);
    assertSame(gbl.getCell(2, 1), cell4);
    assertSame(gbl.getCell(2, 2), cell4);
    
    gbl.removeWidget(cell1.getWidget());
    assertNull(gbl.getCell(0, 0).getWidget());
    assertSame(gbl.getCell(0, 1), cell2);
    assertNull(gbl.getCell(0, 2).getWidget());
    assertNull(gbl.getCell(1, 0).getWidget());
    assertNull(gbl.getCell(1, 1).getWidget());
    ...
    List<List<Integer>> visitedCoords = gbl.visitByRows(new GridBagLayoutVisitor<Integer, List<Integer>>() {
      ...
      public Integer visitCell(GridBagLayoutCell c) {
        ...
        if (c.getColumn() == 0 & c.getRow() == 0)
          assertSame(c.getWidget(), cell1.getWidget());
        if (c.getColumn() == 1 & c.getRow() == 0)
          assertSame(c.getWidget(), cell2.getWidget());
        if (c.getRow() == 1)
          assertSame(c.getWidget(), cell3.getWidget());
  ...
  public void testInsertRow() {
    try {
      ...
      l.add(cell2);
      l.insertRow(1);
      assertNull(l.getCell(1, 0).getWidget());
      assertNull(l.getCell(1, 1).getWidget());
  ...
  private void assertEmptyCells(GridBagLayout gbl, int row, int column, int rowSpan, int columnSpan) {
    for (int iRow = row; iRow - row < rowSpan; iRow++) {
      for (int iCol = column; iCol - column < columnSpan; iCol++) {
        assertNull(gbl.getCell(iRow, iCol).getWidget());
      }
    }
  }
  ...
  private GridBagLayoutCell createCell(Integer row, Integer column, Integer rowspan, Integer colspan, Widget widget, GridBagLayout gridBoxLayout) {  
    return new GridBagLayoutCellImpl(row, column, rowspan, colspan, widget);
  }
